...
def load_df(output_dir):
    df = pd.read_csv(f'{output_dir}/eval_result.csv',header=None)
    df.columns = ['data_name', 'model', 'generation_mode', 'subject', 'method', 'lang', 'num_total','num_correct', 'acc']
    df = df.drop_duplicates(subset=['data_name', 'model', 'generation_mode', 'subject', 'method', 'lang'], keep='last')
...
def post_process(df, data_name, output_dir):
    ...
    df_new = df_new.groupby(['data_name','model','generation_mode','lang']).agg({'num_total': 'sum', 'num_correct': 'sum'}).reset_index()
    df_new['acc'] = df_new['num_correct'] / df_new['num_total']
    df_new = df_new.drop(columns=['num_total', 'num_correct'])
    df_new = df_new.pivot_table(index=['data_name','model','generation_mode'],columns='lang',values='acc')
    ...
    df_new = df_new[['data_name','model','generation_mode'] + langs + ['avg']]
    df_new = df_new.drop(columns=["data_name"])
    df_new.to_csv(f'{output_dir}/{data_name}/eval_{data_name}_pivot.csv', index=False)
